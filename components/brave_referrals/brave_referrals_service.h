...
class BraveReferralsService {
 ...
 private:
  ...
  std::string BuildReferralActivityCheckPayload() const;
  void FetchReferralHeaders();
  bool ShouldCheckReferralActivity() const;
  void CheckReferralActivity();
  ...
  // Invoked from SimpleURLLoader after referral activity check load
  // completes.
  void OnReferralActivityCheckLoadComplete(
      std::unique_ptr<std::string> response_body);
  ...
  std::unique_ptr<network::SimpleURLLoader> referral_activity_check_loader_;
